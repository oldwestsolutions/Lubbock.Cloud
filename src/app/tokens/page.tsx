      ...
      <div className="col-span-3 sm:col-span-3">
        <div className="flex items-center gap-3">
          <div className={cn(
            ...
            token.vendor === 'AMD' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-green-500/10 text-green-400 border border-green-500/20'
          )}>
            {token.gpuModel.slice(0, 4)}
          </div>
          <div>
            <p className="font-medium text-sm">{token.ticker}</p>
            <p className="text-xs text-tech-gray-500">{token.vendor} {token.gpuModel}</p>
          </div>
        </div>
      </div>
...
export default function TokensPage() {
  ...
  const filtered = filter === 'all'
    ? COMPUTE_TOKENS
    : COMPUTE_TOKENS.filter((t) => t.vendor === filter);
    ...
    <main>
      ...
      <div className="page-container">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
          <div>
            <h1 className="section-heading">Compute Tokens</h1>
            ...
            {(['all', 'AMD', 'NVIDIA'] as const).map((f) => (
              <button
                ...
                className={cn(
                  ...
                  filter === f ? 'bg-white/10 text-white' : 'text-tech-gray-500 hover:text-tech-gray-300 hover:bg-white/5'
                )}
              >
                {f === 'all' ? 'All Tokens' : f}
              </button>
        ...
        <div className="mt-16">
          <h2 className="text-xl font-semibold mb-6">Token Mechanics</h2>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="glow-card p-6">
              <h3 className="font-medium">Time-Based</h3>
              <p className="text-sm text-tech-gray-400 mt-2">
                Each token represents exactly 1 GPU-hour of compute on its backing hardware. Redeem tokens to launch sessions, run batch jobs, or consume API credits.
              </p>
            </div>
            <div className="glow-card p-6">
              <h3 className="font-medium">Memory-Tiered</h3>
              <p className="text-sm text-tech-gray-400 mt-2">
                Token price reflects the HBM tier of the GPU. Higher-memory accelerators like MI355X (288 GB) command a premium over H100 (80 GB).
              </p>
            </div>
            <div className="glow-card p-6">
              <h3 className="font-medium">Priority Levels</h3>
              <p className="text-sm text-tech-gray-400 mt-2">
                Standard tokens enter the shared queue. Priority tokens get dedicated scheduling. Reserved tokens guarantee hardware allocation within minutes.
              </p>
            </div>
          </div>
        </div>
